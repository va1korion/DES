...
uint64_t des(uint64_t input, uint64_t key, char mode) {
    ...
    L = (uint32_t) ((init_perm_res & H64_MASK) >> 32);
    ...
    for (i = 0; i < 16; i++) {
        ...
        for (j = 0; j < 8; j++) {
            ...
            s_output <<= 4;
            s_output |= (uint32_t) (S[j][row][column] & 0x0f);
...
int main(int argc, const char * argv[]) {
    ...
    uint64_t input = 0x52849112DEAFCEED;
    uint64_t key = 0xFEEED15EA5E01234;
